...
for version in range(1, 45):  # Create 44 versions
    tokens_to_remove = set([token for token, _ in sorted_tokens[:version]])
    ...
    for method in ['skip', 'mask']:
        ...
        torch.save(train_t, f'{model_process_path}/{token_len}_train_data_filtered.pkl')
        torch.save(torch.LongTensor(train_label), f'{model_process_path}/{token_len}_train_label_filtered.pkl')
        torch.save(dev_t, f'{model_process_path}/{token_len}_dev_data_filtered.pkl')
        torch.save(torch.LongTensor(dev_label), f'{model_process_path}/{token_len}_dev_label_filtered.pkl')
        torch.save(test_t, f'{model_process_path}/{token_len}_test_data_filtered.pkl')
        torch.save(torch.LongTensor(test_label), f'{model_process_path}/{token_len}_test_label_filtered.pkl')
        torch.save(composer2id, f'{model_process_path}/composer2id_map_filtered.pkl')
