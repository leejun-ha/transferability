...
def generate_and_save_plot(model_name, token_range_data, model_performance_data, output_dir, model_replace):
    ...
    counts = list(token_range_data.values())
    token_ranges = list(token_range_data.keys())
    ...
    # Midpoints for token ranges for bar graph alignment (based on your ranges)
    midpoints = [32, 64, 128, 256, 384, 512]

    # Create the plot
    fig, ax1 = plt.subplots(figsize=(12, 6))

    # Bar chart for token distribution
    ax1.bar(midpoints, counts, width=150, color='lightblue', alpha=0.6)
