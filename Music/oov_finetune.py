...
def main():
    ...
    parser.add_argument('--oov_version', type=int, default=0, help='OOV version to use (0-44)')
    parser.add_argument('--oov_method', type=str, choices=['skip', 'mask'], default='skip', help='OOV handling method')
    ...
    if args['oov_version'] > 0:
        args['filename'] += f'_oov{args["oov_version"]}_{args["oov_method"]}'
...
def train(args):
    ...
    # Load the appropriate dataset based on OOV version and method
    if args['oov_version'] > 0:
        data_path = os.path.join(args['datadir'], f'version_{args["oov_version"]}_{args["oov_method"]}')
        data = torch.load(os.path.join(data_path, f'{args["token_len"]}_train_data_filtered.pkl'))
        label = torch.load(os.path.join(data_path, f'{args["token_len"]}_train_label_filtered.pkl'))
    ...
    # Adjust the model's embedding layer if using masked OOV
    if args['oov_method'] == 'mask' and args['oov_version'] > 0:
        # Assuming the mask token is represented by the maximum value in the vocabulary
        mask_token_id = data.max().item()
        model.resize_token_embeddings(mask_token_id + 1)
